...
--[[
    class Cone: inherits from (and implements) ShapeObject
...
function Cone(arg)  -- both constructors in one function. Pass either a Vec4f color for interactive draw, or an existing Cone to copy
    ...
    local cone = ShapeObject(rawcone, Transform{ orientation = AngleAxis(Degrees(-90), Axis{1.0, 0.0, 0.0}) })
...
function Cone_removeObject(cone)
    World:removeChild(cone.attach_here)
end
